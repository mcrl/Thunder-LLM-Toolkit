...
from thunderllm.inference.api.group import ConfigurableGroup
...
from thunderllm.inference.utils import positional_deprecated, eval_logger
        ...
        for (metric, filter_key), items in self.sample_metrics.items():
            ...
            if isinstance(bootstrap_iters, int):
                ...
                self.agg_metrics[f"{metric}_stderr,{filter_key}"] = (
                    stderr_fn(items) if (
                        stderr_fn and len(items) > 1) else "N/A"
                )
            else:
                raise ValueError(
