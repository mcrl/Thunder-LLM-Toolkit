...
from thunderllm.inference.loggers.utils import _handle_non_serializable, remove_none_pattern


logger = logging.getLogger(__name__)

def get_wandb_printer():
    return None
    ...
    def _generate_dataset(
        ...
        for metric in metrics_list:
            ...
            if metric in ["word_perplexity", "byte_perplexity", "bits_per_byte"]:
                metrics[f"{metric}_loglikelihood"] = [x[metric][0]
                                                      for x in data]
        ...
        if config["output_type"] == "loglikelihood":
            ...
            instance = [x["arguments"][0][0] for x in data]
            choices = [
                "\n".join([f"{idx}. {y[1]}" for idx,
                          y in enumerate(x["arguments"])])
                for x in data
            ]
