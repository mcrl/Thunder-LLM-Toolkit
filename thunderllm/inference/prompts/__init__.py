...
from thunderllm.inference.utils import eval_logger
...
def get_prompt(prompt_id: str, dataset_name: str = None, subset_name: str = None):
    ...
    eval_logger.info(
        f"Loading prompt from {category_name} for {dataset_full_name}")
...
def load_prompt_list(
    ...
    if category_name == "promptsource":
        ...
        prompt_list = utils.pattern_match(
            prompt_name, prompts.all_template_names)

    elif ".yaml" in category_name:
        import yaml

        if yaml_path is not None:
            category_name = os.path.realpath(
                os.path.join(yaml_path, category_name))
