...
from thunderllm.inference.api import samplers
from thunderllm.inference.api.instance import Instance, OutputType
from thunderllm.inference.api.metrics import bits_per_byte, mean, weighted_perplexity
from thunderllm.inference.api.registry import (
...
from thunderllm.inference.caching.cache import load_from_cache, save_to_cache
from thunderllm.inference.filters import build_filter_ensemble
from thunderllm.inference.prompts import get_prompt
    ...
    def to_dict(self, keep_callable: bool = False) -> dict:
        ...
        for k, v in list(cfg_dict.items()):
            if v is None:
                ...
                cfg_dict[k] = self.serialize_function(
                    v, keep_callable=keep_callable)
        return cfg_dict
...
class Task(abc.ABC):
    ...
    def __init__(
        ...
        self._config: TaskConfig = TaskConfig(
            {**config}) if config else TaskConfig()
    ...
    def __init__(
        ...
        if self.config.metric_list is None:
            ...
            for metric_name in _metric_list:
                ...
                self._higher_is_better[metric_name] = is_higher_better(
                    metric_name)
        else:
            for metric_config in self.config.metric_list:
                ...
                if "aggregation" in metric_config:
                    ...
                    INV_AGG_REGISTRY = {v: k for k,
                                        v in AGGREGATION_REGISTRY.items()}
        ...
        if self.config.filter_list is not None:
            ...
            for filter_config in self.config.filter_list:
                ...
                filter_pipeline = build_filter_ensemble(
                    filter_name, components)
                self._filters.append(filter_pipeline)
        else:
            self._filters = [build_filter_ensemble(
    ...
    def construct_requests(
        ...
        if self.OUTPUT_TYPE == "loglikelihood":
            ...
            if self.multiple_input:
                ...
                arguments = [(ctx, f"{target_delimiter}{cont}")
                             for cont in choices]
    ...
    def process_results(self, doc, results):
        ...
        if self.OUTPUT_TYPE == "loglikelihood":
            ...
            if "acc_mutual_info" in use_metric:
                ...
                acc_mutual_info = 1.0 if np.argmax(
                    lls_mutual_info) == gold else 0.0
