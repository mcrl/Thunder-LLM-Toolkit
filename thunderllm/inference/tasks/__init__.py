...
from thunderllm.inference.api.group import ConfigurableGroup, GroupConfig
from thunderllm.inference.api.task import ConfigurableTask, Task
from thunderllm.inference.evaluator_utils import get_subtask_list
...
class TaskManager:
    """TaskManager indexes all tasks from the default `thunderllm.inference/tasks/`
    ...
    def initialize_tasks(
        ...
        :param include_defaults: bool = True
            If set to false, default tasks (those in thunderllm.inference/tasks/) are not indexed.
        :return
    ...
    def list_all_tasks(
        ...
        def sanitize_path(path):
            # don't print full path if we are within the thunderllm.inference/tasks dir !
            # if we aren't though, provide the full path.
            if "thunderllm.inference/tasks/" in path:
                return "thunderllm.inference/tasks/" + path.split("thunderllm.inference/tasks/")[-1]
            else:
                return path
        ...
        for t in self.all_subtasks:
            ...
            if path != -1:
                ...
                if "output_type" in config:
                    ...
                    include_config = utils.load_yaml_config(
                        include_path, mode="simple")
    ...
    def _load_individual_task_or_group(
        ...
        def _process_group_config(config, update_config=None):
            ...
            group_config = {k: v for k,
                            v in config.items() if k in GROUP_ONLY_KEYS}
                    ...
                    if self._name_is_tag(name_or_config):
                        ...
                        return dict(
                            collections.ChainMap(
                                *map(fn, reversed(subtask_list)))
                        )
                    else:
                        group_name = ConfigurableGroup(
                            config={"group": name_or_config,
                                    "task": subtask_list}
                        )
        ...
        return {
            group_name: dict(collections.ChainMap(
                *map(fn, reversed(subtask_list))))
        }
    ...
    def load_task_or_group(self, task_list: Optional[Union[str, list]] = None) -> dict:
        ...
        all_loaded_tasks = dict(
            collections.ChainMap(
                *map(self._load_individual_task_or_group, task_list))
        )
                            ...
                            if attr in config:
                                ...
                                for tag in attr_list:
                                    if tag not in tasks_and_groups:
                                        ...
                                        tasks_and_groups[tag]["task"].append(
                                            task)
                    else:
                        self.logger.debug(
                            f"File {f} in {root} could not be loaded")
...
def _check_duplicates(task_dict: dict) -> List[str]:
    ...
    Takes the output of thunderllm.inference.evaluator_utils.get_subtask_list and
...
def get_task_dict(
    ...
    string_task_name_list = [
        task for task in task_name_list if isinstance(task, str)]
