...
def _drop_duplicates_in_input(untokenized_dataset):
    ...
    untokenized_dataset = untokenized_dataset.select(
        indices_to_keep).flatten_indices()
...
class _SCROLLSTask(ConfigurableTask):
    ...
    def __init__(self, config=None):
        ...
        if self.DATASET_NAME is not None:
            self.metric = load_metric(
                _download_metric(), config_name=self.DATASET_NAME)
    ...
    def training_docs(self):
        ...
        processed_docs = [
            item for sublist in processed_docs for item in sublist]
        processed_dict = {
    ...
    def download(self, *args, **kwargs):
        ...
        for split in self.dataset:
            self.dataset[split] = _drop_duplicates_in_input(
                self.dataset[split])
    ...
    def prune(self):
        ...
        self.dataset = self.dataset.filter(
            _filter, num_proc=self.PRUNE_NUM_PROC)
...
class QuALITY(_SCROLLSMultipleChoiceTask):
    ...
    def _process_doc(self, doc):
        ...
        doc["gold"] = doc["choices"].index(
            QuALITY._normalize_answer(doc["outputs"][0]))
