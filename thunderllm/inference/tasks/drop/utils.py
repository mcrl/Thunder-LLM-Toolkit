...
def get_answers(doc):
    ...
    answers = []
    answers_set = set()
    candidates = [doc["answer"]] + \
        _flatten_validated_answers(doc["validated_answers"])
...
def parse_answer(answer):
    ...
    return (
        " ".join(
            [answer["date"]["day"], answer["date"]
                ["month"], answer["date"]["year"]]
        ).strip(),
    )
...
def _normalize(answer):
    tokens = [
        _white_space_fix(_remove_articles(
            _fix_number(_remove_punc(token.lower()))))
