...
from thunderllm.inference.filters.extraction import Filter, RegexFilter
...
class MultiChoiceRegexFilter(ExtendedRegexFilter):
    ...
    def apply(self, resps, docs):
        ...
        for r, doc in zip(resps, docs):
            ...
            without_paren_fallback_regex = "|".join(
                without_paren_fallback_regexes)
            ...
            for resp in r:
                ...
                if not match:
                    match = self.find_match(
                        fallback_regex, self.filter_ignores(
                            resp), choice_to_alpha
                    )
                    if not match:
                        match = self.find_match(
