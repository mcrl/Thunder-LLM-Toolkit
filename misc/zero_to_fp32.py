...
def get_checkpoint_files(checkpoint_dir, glob_pattern):
    ...
    ckpt_files = sorted(glob.glob(os.path.join(
        checkpoint_dir, glob_pattern)), key=natural_keys)
...
def _zero3_merge_frozen_params(state_dict, world_size, zero_model_states):
    ...
    if debug:
        ...
        wanted_numel = sum(s.numel() for s in frozen_param_shapes.values())
        avail_numel = sum(
            [p.numel() for p in zero_model_states[0].frozen_param_fragments.values()]) * world_size
        print(f'Frozen params: Have {avail_numel} numels to process.')
        print(
            f'Frozen params: Need {wanted_numel} numels in {wanted_params} params')
...
def _zero3_merge_trainable_params(state_dict, world_size, fp32_flat_groups, zero_model_states):
    ...
    for name, shape in param_shapes.items():
        ...
        state_dict[name] = torch.cat(
            tuple(fp32_flat_groups[i].narrow(
                0, offset, partitioned_numel) for i in range(world_size)),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--exclude_frozen_parameters",
                        action='store_true', help="exclude frozen parameters")
    parser.add_argument(
